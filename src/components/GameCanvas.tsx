...
export function GameCanvas() {
  ...
  const handleUpgrade = useCallback((type: 'damage' | 'fireRate' | 'maxHealth' | 'healthRegen' | 'range' | 'goldPerRound' | 'interest') => {
    if (gameRef.current) {
  ...
  const handlePurchasePermanentUpgrade = (type: keyof PermanentUpgrades) => {
    // Calculate cost
    const upgradeInfo: Record<keyof PermanentUpgrades, { baseCost: number; scaling: number; max: number }> = {
    ...
    const cost = Math.floor(info.baseCost * Math.pow(info.scaling, currentLevel));
